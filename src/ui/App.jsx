  ...
  return <div className="sticky top-0 z-40 glass shadow-elev">
    <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <span className="font-extrabold tracking-tight">FinPath</span>
      </div>
      <div className="flex items-center gap-2">
        {["home","tracks","progress","account"].map(k=>(<Button key={k} variant="ghost" onClick={()=>onNav(k)}>{k[0].toUpperCase()+k.slice(1)}</Button>))}
    ...
    <Card className="p-8 flex items-center justify-between gap-8">
      <div className="space-y-3">
        ...
        <div className="flex gap-3">
          ...
          <Button variant="outline" onClick={onContinue}>Browse tracks</Button>
        </div>
      </div>
      <div className="hidden md:block text-center">
        <div className="relative grid place-items-center">
          <ProgressRing value={pct} />
          <div className="absolute text-sm font-semibold">{Math.round(pct)}%</div>
        </div>
      ...
      <Card key={t.id} className="p-6">
        ...
        {t.chapters.map(c=>(
          <div key={c.id} className="mb-5">
            <p className="text-xs uppercase tracking-wide text-muted mb-2">{c.title}</p>
            {(c.lessons||[]).map(l=>(
              <button key={l.id} onClick={()=>onOpenLesson(t.id,c.id,l.id)} className="w-full text-left py-2 px-3 rounded-xl hover:bg-[var(--fg)]/10 transition">
                {l.title}
              </button>
            ))}
          </div>
...
function Questionnaire({ questions, onFinish }){
  ...
  const choose=(val)=>{ const next=[...answers]; next[i]=val; setAnswers(next); setI(i+1); };
  if(done) return <div className="max-w-lg mx-auto px-6 py-16"><Card className="p-8 text-center"><div className="text-6xl mb-2">🧭</div><h2 className="text-2xl font-extrabold mb-2">You're set!</h2><p className="text-muted mb-4">We’ll recommend a starting point based on your answers.</p><Button onClick={()=>onFinish(answers.filter(Boolean).length)}>See recommendation</Button></Card></div>;
  return <div className="max-w-lg mx-auto px-6 py-16">
    <Card className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div className="text-sm text-muted">Question {i+1} of {questions.length}</div>
        <div className="w-40"><ProgressBar value={(i/questions.length)*100} /></div>
      </div>
      <h2 className="text-2xl font-extrabold tracking-tight min-h-[72px]">{q.text}</h2>
      <div className="mt-8 grid grid-cols-2 gap-3">
        <Button onClick={()=>choose(true)}>{q.labels?.[0]||"Yes"}</Button>
        <Button variant="outline" onClick={()=>choose(false)}>{q.labels?.[1]||"No"}</Button>
      </div>
    </Card>
  </div>;
}
...
function Lesson({ tracks, tid, cid, lid, onExit, onAward, onPracticeSaved }){
  ...
  const finish=()=>{
    if(lesson.practice){
      ...
      setComplete(true); try{ localStorage.setItem(key, JSON.stringify({ complete:true, answers })); }catch{}
      return;
    }
  ...
  return <div className="max-w-3xl mx-auto px-6 py-12">
    ...
    <div className="mb-6 flex items-start justify-between">
      <div><p className="text-xs uppercase tracking-wide text-muted">{track.name} • {chapter.title}</p><h2 className="text-3xl font-extrabold tracking-tight">{lesson.title}</h2></div>
      <Badge>{lesson.practice? "Practice": `${lesson.xp||10} XP`}</Badge>
    </div>
    {!lesson.practice && (<Card className="p-8">
    ...
    <Card className="p-6 mt-6">
      <h3 className="font-semibold mb-3">{lesson.practice? "Practice Questions" : "Quick Check"}</h3>
      {(lesson.quiz||[{q:"Ready to start?", labels:["Start","Cancel"]}]).map((q,i)=>(
        <div key={i} className="mb-4">
          <p className="mb-2">{q.q || q}</p>
          <div className="grid grid-cols-2 gap-2">
            <Button onClick={()=>setAns(i,true)} variant={answers[i]===true?"primary":"outline"} aria-pressed={answers[i]===true}>{(q.labels&&q.labels[0])||"Yes"}</Button>
            <Button onClick={()=>setAns(i,false)} variant={answers[i]===false?"primary":"outline"} aria-pressed={answers[i]===false}>{(q.labels&&q.labels[1])||"No"}</Button>
          </div>
        </div>
      ))}
    </Card>
    <div className="mt-6 flex items-center gap-3">
      <Button onClick={finish} disabled={!lesson.practice && !Object.keys(answers).length}>{lesson.practice? "Submit Quiz" : (complete? "Close" : "Finish Lesson")}</Button>
...
function Progress({ onBack }){
  ...
  const weekly = s?.game?.weekly || { completed:0, target:5 };
  const badges = s?.game?.badges || []; const certs = s?.game?.certificates || [];
  return <div className="max-w-5xl mx-auto px-6 py-12">
    ...
    <Card className="p-6 mt-6">
      <p className="text-sm text-muted mb-3">Badges</p>
      <div className="flex flex-wrap gap-2">{badges.length===0? <span className="text-sm text-muted">No badges yet</span> : badges.map(b=><Badge key={b}>{b}</Badge>)}</div>
      <div className="mt-6"><p className="text-sm text-muted mb-3">Certificates</p>{certs.length===0? <span className="text-sm text-muted">No certificates yet</span> : (<div className="grid md:grid-cols-2 gap-3">{certs.map((c,idx)=>(<Card key={idx} className="p-4"><div className="flex items-center justify-between"><div><div className="font-semibold">{c.trackName}</div><div className="text-xs text-muted">{new Date(c.date).toLocaleDateString()} • {c.scorePct}%</div></div><Badge>Passed</Badge></div></Card>))}</div>)}</div>
    </Card>
  </div>;
}
...
export default function App(){
  ...
  return <div className="min-h-screen">
    <div className="sticky top-0 z-40 glass shadow-elev">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg" style={{background:"var(--fg)"}}/><span className="font-extrabold tracking-tight">FinPath</span></div>
        <div className="flex items-center gap-2">{["home","tracks","progress","account"].map(k=>(<Button key={k} variant="ghost" onClick={()=>setRoute(k)}>{k[0].toUpperCase()+k.slice(1)}</Button>))}<Button variant="ghost" onClick={()=>setTheme(theme==="dark"?"light":"dark")} aria-label="Toggle theme">{theme==="dark"?"Light":"Dark"}</Button></div>
      </div>
    </div>
